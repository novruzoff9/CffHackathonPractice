...
### ?? Regular User (Pre-seeded)
- **Email:** `user@test.com`
- **Password:** `User123`
- **Username:** `user`
- **Role:** User
- **Permissions:** Can browse products/categories and create orders
...
```sql
-- First, register the user normally, then find the user ID
SELECT "Id", "Email" FROM "AspNetUsers" WHERE "Email" = 'newadmin@example.com';

-- Then assign Admin role
INSERT INTO "AspNetUserRoles" ("UserId", "RoleId")
SELECT 
    (SELECT "Id" FROM "AspNetUsers" WHERE "Email" = 'newadmin@example.com'),
    (SELECT "Id" FROM "AspNetRoles" WHERE "Name" = 'Admin');
```

**Option 2: Register normally and update via database**
1. Register a new user via `/api/Auth/register`
2. Use the SQL above to assign Admin role
3. User needs to login again to get a new token with Admin claims
